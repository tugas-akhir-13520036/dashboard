...
const AttributeListAuthority = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await attributesList();
        setAttributes(data);
      } catch (error) {
    ...
    return (
      <Container>
        <Typography variant="h6" color="error">
          Error fetching attribute list: {error}
        </Typography>
      </Container>
    );
  }
...
export default AttributeListAuthority;
